...
{
    // Wait for IRC to get it first - disabled with dzc
...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "dzc " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
...
// testnet dns seed begins with 't', all else are dzc dns seeds.
static const char *strDNSSeed[][1] = {
    // NOTE: Only testnet nodes are included in this build. If you are using this
    // for a real dzc offering, you will want to remove the "t" that
...
void ThreadDNSAddressSeed2(void* parg)
{
    ...
    if (true /*!fTestNet*/)  // DZC enables dns seeding with testnet too
    {
...
void static ThreadIssueOldCurrency(void* parg)
{
    CWallet* pwallet = (CWallet*)parg;
    try
    {
        vnThreadsRunning[THREAD_ISSUINGOLDCURRENCY]++;
        IssueOldCurrency(pwallet);
        vnThreadsRunning[THREAD_ISSUINGOLDCURRENCY]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_ISSUINGOLDCURRENCY]--;
        PrintException(&e, "ThreadIssueOldCurrency()");
    } catch (...) {
        vnThreadsRunning[THREAD_ISSUINGOLDCURRENCY]--;
        PrintException(NULL, "ThreadIssueOldCurrency()");
    }
...
bool BindListenPort(string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, sizeof(sockaddr)) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to port %d on this computer.  dzc is probably already running."), ntohs(sockaddr.sin_port));
        else
...
void StartNode(void* parg)
{
    ...
    if(GetBoolArg("-genpos", false)){
        // DZC: mint proof-of-stake blocks in the background
    ...
    if (!CreateThread(ThreadIssueOldCurrency, pwalletMain))
        printf("Error: CreateThread(ThreadIssueOldCurrency) failed\n");
//#ifdef WIN32
//    printf("Info: Proof-Of-Stake Generation Disabled\n");
//#else
//    if(GetBoolArg("-genpos", false)){
...
bool StopNode()
{
    ...
    if (vnThreadsRunning[THREAD_ISSUINGOLDCURRENCY] > 0) printf("ThreadIssueOldCurrency still running\n");
