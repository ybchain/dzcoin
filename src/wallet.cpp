...
// DZC: optional setting to unlock wallet for block minting only;
...
bool CWallet::AddToWalletElse(const CWalletTx &wtxIn)
{
    uint256 hash = wtxIn.GetHash();
    LOCK(cs_wallet);
    mapWalletElse.insert(make_pair(hash, wtxIn));
}
...
int64 CWallet::GetBalance(unsigned int currencyId) const
{
    int64 nTotal = 0;
    {
        LOCK(cs_wallet);
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            if (!pcoin->IsFinal() || !pcoin->IsConfirmed() || pcoin->currency.id != currencyId)
                continue;
...
int64 CWallet::GetUnconfirmedBalance(unsigned int currencyId) const
{
    int64 nTotal = 0;
    {
        LOCK(cs_wallet);
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            if ((pcoin->IsFinal() && pcoin->IsConfirmed()))
                continue;
            if (pcoin->currency.id == currencyId)
                nTotal += pcoin->GetAvailableCredit();
        }
    }
    return nTotal;
...
// DZC: total coins staked (non-spendable until maturity)
...
bool CWallet::SelectCoinsMinConf(int64 nTargetValue, unsigned int nSpendTime, int nConfMine, int nConfTheirs, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64& nValueRet, unsigned int currencyId) const
{
    setCoinsRet.clear();
    nValueRet = 0;
...

    {
       ...
       {
           const CWalletTx* pcoin = &(*it).second;
           unsigned int curId = pcoin->currency.id;
           if(currencyId == curId)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // DZC: timestamp must not exceed spend time
...
bool CWallet::SelectCoins(int64 nTargetValue, unsigned int nSpendTime, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64& nValueRet, const CurrencyInfo &currency) const
{
    unsigned int currencyId = currency.fPOI ? 0 : currency.id;
    return (SelectCoinsMinConf(nTargetValue, nSpendTime, 1, 6, setCoinsRet, nValueRet, currencyId) ||
            SelectCoinsMinConf(nTargetValue, nSpendTime, 1, 1, setCoinsRet, nValueRet, currencyId) ||
            SelectCoinsMinConf(nTargetValue, nSpendTime, 0, 1, setCoinsRet, nValueRet, currencyId));
}
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            while (true)
            {
                ...
                int64 nValueIn = 0;
                if (!SelectCoins(nTotalValue, wtxNew.nTime, setCoins, nValueIn, wtxNew.currency))
                    return false;
                ...
                // DZC: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    //  post-backup change.
#ifdef WIN32
                    if (!GetBoolArg("-avatar", true)) // DZC: not avatar mode; dzc: avatar mode enabled by default to avoid change being sent to hidden address
#else
...
// DZC: create coin stake transaction
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64 nSearchInterval, CTransaction& txNew)
{
    ...
    if (!SelectCoins(nBalance - nReserveBalance, txNew.nTime, setCoins, nValueIn, currency))
        return false;
    if (setCoins.empty())
        return false;
    int64 nCredit = 0;
...
// DZC: check 'spent' consistency between wallet and txindex
// DZC: fix wallet spent state according to txindex
...
// DZC: disable transaction (only for coinstake)
...
std::map<unsigned int, CurrencyInfo> CWallet::getIssuedCurrencies()
{
    std::map<unsigned int, CurrencyInfo> mapCurrency;
    {
        LOCK(cs_wallet);
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            addToMapCurrency(mapCurrency, it);
        }
        for (map<uint256, CWalletTx>::const_iterator it = mapWalletElse.begin(); it != mapWalletElse.end(); ++it)
        {
            addToMapCurrency(mapCurrency, it);
        }
    }
...
std::map<unsigned int, CurrencyInfo> CWallet::getMyCurrencies()
{
    std::map<unsigned int, CurrencyInfo> mapCurrency;
    {
        LOCK(cs_wallet);
...
void CWallet::addToMapCurrency(std::map<unsigned int, CurrencyInfo> &mapCurrency, std::map<uint256, CWalletTx>::const_iterator it)
{
    const CWalletTx* pcoin = &(*it).second;
    if (!pcoin->IsFinal() || !pcoin->IsConfirmed())
        return;

    unsigned int key = pcoin->currency.id;
    map<unsigned int, CurrencyInfo>::iterator mi = mapCurrency.find(key);
    if (mi == mapCurrency.end())
    {
        CurrencyInfo curInfo(pcoin->currency);
        mapCurrency.insert(std::make_pair(key, curInfo));
    }
}
