...
Object blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool fPrintTransactionDetail)
{
    ...
    result.push_back(Pair("flags", strprintf("%s%s", blockindex->IsProofOfIssue() ? "proof-of-issue" : (blockindex->IsProofOfStake()? "proof-of-stake" : "proof-of-work"), blockindex->IsProofOfIssue() ? "" : (blockindex->GeneratedStakeModifier()? " stake-modifier": ""))));
...
// DZC: get network Gh/s estimate
// DZC note: this is only useful during the initial proof-of-work 'IPO' phase of the network
...
void ListMyCurrencies(Array& ret)
{
    map<unsigned int, CurrencyInfo> mapCurrency = pwalletMain->getMyCurrencies();
    ...
    for (map<unsigned int, CurrencyInfo>::const_iterator it = mapCurrency.begin(); it != mapCurrency.end(); ++it)
    {
        ...
        entry.push_back(Pair("issuedividespend", curInfo->issuedividespend));
        entry.push_back(Pair("balance", ValueFromAmount(pwalletMain->GetBalance(curInfo->id))));
        ret.push_back(entry);
    }
}
...
Value getinfo(const Array& params, bool fHelp)
{
    ...
    Array currencies;
    ListMyCurrencies(currencies);
    obj.push_back(Pair("my currencies", currencies));
...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new DZC address for receiving payments.  "
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Returns the current DZC address for receiving payments to this account.");
...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount <dzcaddress> <account>\n"
            "Sets the account associated with the given address.");

    CBitcoinAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid DZC address");
...
Value edittaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "edittaccount <dzcaddress> <account>\n"
            "Sets the account associated with the given address.");
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 2 || params.size() > 4))
        throw runtime_error(
            "sendtoaddress <dzcaddress> <amount> [comment] [comment-to]\n"
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage <dzcaddress> <message>\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage <dzcaddress> <signature> <message>\n"
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress <dzcaddress> [minconf=1]\n"
            "Returns the total amount received by <dzcaddress> in transactions with at least [minconf] confirmations.");
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 3 || params.size() > 6))
        throw runtime_error(
            "sendfrom <fromaccount> <todzcaddress> <amount> [minconf=1] [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.000001\n"
            "requires portfolio passphrase to be set with walletpassphrase first");
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() < 3 || params.size() > 6))
        throw runtime_error(
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(-5, string("Invalid DZC address:")+s.name_);
...
Value distribute(const Array& params, bool fHelp)
{
    ...
    else
    {
        ...
        {
            Object obj;
            obj.push_back(Pair("dzc_address", distribution.GetPeershareAddress().ToString()));
...
Value walletpassphrase(const Array& params, bool fHelp)
{
    ...
    // DZC: if user OS account compromised prevent trivial sendmoney commands
    if (params.size() > 2)
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "Portfolio encrypted; DZC server stopping. Please restart server to run with encrypted portfolio";
}
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress <dzcaddress>\n"
            "Return information about <dzcaddress>.");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "DZC is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "DZC is downloading blocks...");
    ...
    if (params.size() == 0)
    {
            ...
            (nTransactionsUpdated != nTransactionsUpdatedLast && GetTime() - nStart > 60))
        {
            ...
            // Create new block
            pblock = CreateNewBlock(*pMiningKey, pwalletMain, currency, false);
            if (!pblock)
...
// DZC: get information of sync-checkpoint
...
// DZC: reserve balance from being staked for network protection
...
// DZC: make a public-private key pair
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: dzc-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
