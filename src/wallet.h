...
{
private:
    bool SelectCoinsMinConf(int64 nTargetValue, unsigned int nSpendTime, int nConfMine, int nConfTheirs, std::set<std::pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64& nValueRet, unsigned int currencyId=0) const;
    bool SelectCoins(int64 nTargetValue, unsigned int nSpendTime, std::set<std::pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64& nValueRet, const CurrencyInfo &currency) const;
...
public:
    ...
    std::string strCommonWalletFile;
    ...
    CWallet(std::string strWalletFileIn)
    {
        ...
        if(GetBoolArg("-testnet", false))
            strCommonWalletFile = /*std::string("../../common/testnet/") +*/ strWalletFileIn;
        else
            strCommonWalletFile = /*std::string("../common/") +*/ strWalletFileIn;
        fFileBacked = true;
        nMasterKeyMaxID = 0;
        pwalletdbEncryption = NULL;
    }
    ...
    std::map<uint256, CWalletTx> mapWalletElse;
    ...
    bool AddToWalletElse(const CWalletTx& wtxIn);
    ...
    int64 GetBalance(unsigned int currencyId=0) const;
    int64 GetUnconfirmedBalance(unsigned int currencyId=0) const;
    ...
    //dzc:get issued currencies
    std::map<unsigned int, CurrencyInfo> getIssuedCurrencies();
    std::map<unsigned int, CurrencyInfo> getMyCurrencies();
    void addToMapCurrency(std::map<unsigned int, CurrencyInfo> &mapCurrency, std::map<uint256, CWalletTx>::const_iterator it);
};
...
// dzc: A reserve key that always returns the default key
